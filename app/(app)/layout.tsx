...
export default async function AppLayout({ children }: { children: ReactNode }) {
  ...
  const workspace = await prisma.workspace.findUniqueOrThrow({
    ...
    select: { id: true, name: true, slug: true, type: true, plan: true, hasShop: true, status: true, actionPinHash: true },
  });
  ...
  return (
    <AppShell
      ...
      workspace={{ id: workspace.id, name: workspace.name, slug: workspace.slug, type: workspace.type, plan: workspace.plan, hasPin: !!workspace.actionPinHash, hasShop: workspace.hasShop }}
      realUser={user}
    >
      {children}
    </AppShell>
