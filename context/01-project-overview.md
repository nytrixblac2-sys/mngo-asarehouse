...
## Current Product State (as of v1.1.1.0, 2026-09-14)
...
**Important — pricing enforcement mostly does not exist yet.** These tiers are still primarily marketing copy on the public `/pricing` page. `Workspace` has a manually admin-toggled `paid: Boolean` (v1.0.1.6) and, as of v1.1.0.8, a real `plan: WorkspacePlan` field (FREE/STARTER/PRO/ENTERPRISE) — the first plan field anywhere in the app meant to actually be enforced, though enforcement is deliberately scoped to STORE-only for now (every pre-existing workspace was grandfathered to `ENTERPRISE` in the same migration, so RENTAL/HOSTEL are unaffected and still have no real limits checked anywhere). STORE's inventory tracking (built v1.1.1.0) is designated Starter-and-up in copy, but **that specific gate isn't wired up yet either** — a Free-tier Store can use it today; the gate is planned alongside the rest of STORE's Free-tier limits (current-month-only financials, no shareable public link) as a dedicated later stage. Out of Scope's "Billing and subscription management" (above) is still true: no payment processor, no self-serve upgrade/downgrade.
