...
export function PropertyForm({
  ...
  isError: boolean;
  error: Error | null;
  /** STORE workspaces call this a "shop," not a "property." */
  workspaceType?: WorkspaceType;
}) {
  const [name, setName] = useState("");
  const isStore = workspaceType === "STORE";
  const noun = isStore ? "shop" : "property";
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.35)" }}>
        <div className="w-full max-w-sm rounded-2xl p-10 flex flex-col items-center gap-3" style={{ background: C.card }}>
          ...
          <p className="text-sm font-semibold" style={{ color: C.text }}>Adding {noun}…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.35)" }}>
      <div className="w-full max-w-sm rounded-2xl p-6" style={{ background: C.card }}>
        <div className="flex items-center justify-between mb-5">
          <p className="text-lg font-bold" style={{ color: C.text }}>Add {noun}</p>
          <button onClick={onClose}><X size={20} style={{ color: C.muted }} /></button>
        </div>
        <label className="text-xs font-semibold" style={{ color: C.muted }}>{isStore ? "Shop name" : "Property name"}</label>
        <input
          value={name}
